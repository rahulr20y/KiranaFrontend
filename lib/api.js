...
export const authAPI = {
    ...
    googleAuth: (idToken, userType) =>
        api.post('/users/google-auth/', { id_token: idToken, user_type: userType }),
};
...
export const productsAPI = {
    ...
    updateStock: (id, data) => 
        api.post(`/products/${id}/update_stock/`, data),
    getAuditLogs: (id) => 
        api.get(`/products/${id}/audit_logs/`),
    getVarianceReport: () => 
        api.get('/products/variance_report/'),
    bulkImport: (file) => {
        const formData = new FormData();
        formData.append('file', file);
        return api.post('/products/bulk_import/', formData, {
...
export const dealersAPI = {
    ...
    getStaff: () => api.get('/dealers/manage_staff/'),
    addStaff: (data) => api.post('/dealers/add_staff/', data),
    updateStaffLocation: (lat, lng) => 
        api.post('/dealers/update_staff_location/', { lat, lng }),
    getStaffLocations: () => 
        api.get('/dealers/get_staff_locations/'),
...
export const ordersAPI = {
    ...
    getRoutePlan: () => api.get('/orders/route-plan/'),
    getOrderStats: () => api.get('/orders/stats/'),
};

// Returns APIs
export const returnsAPI = {
    listReturns: () => api.get('/orders/returns/'),
    getReturn: (id) => api.get(`/orders/returns/${id}/`),
    createReturn: (data) => api.post('/orders/returns/', data),
    approveReturn: (id, data) => api.post(`/orders/returns/${id}/approve/`, data),
    rejectReturn: (id, data) => api.post(`/orders/returns/${id}/reject/`, data),
};

// Notifications APIs
export const notificationsAPI = {
    listBroadcasts: () => api.get('/notifications/broadcasts/'),
    createBroadcast: (data) => api.post('/notifications/broadcasts/', data),
    getLatestBroadcasts: () => api.get('/notifications/broadcasts/latest/'),
    listPersonal: () => api.get('/notifications/notifications/'),
...
export const paymentsAPI = {
    ...
    detailedLedger: (partnerId) => 
        api.get(`/payments/detailed_ledger/?partner_id=${partnerId}`),
    createRazorpayOrder: (data) => api.post('/payments/create_razorpay_order/', data),
    verifyPayment: (data) => api.post('/payments/verify/', data),
    simulateDirectPayment: (data) => api.post('/payments/simulate-direct/', data),
    listCreditLimits: () => api.get('/payments/credit-limits/'),
    createCreditLimit: (data) => api.post('/payments/credit-limits/', data),
    updateCreditLimit: (id, data) => api.patch(`/payments/credit-limits/${id}/`, data),
};
