...
export const generateInvoicePDF = (order) => {
    ...
    doc.text(order.dealer_name || 'Dealer Name', 20, 56);
    doc.setFontSize(8);
    ...
    doc.setFont(undefined, 'normal');
    const shopkeeperName = order.shopkeeper_name || 'Shopkeeper Name';
    ...
    const tableData = order.items.map((item, idx) => [
        ...
        `₹${item.product_price}`,
        `₹${(item.subtotal || item.product_price * item.quantity).toLocaleString()}`
    ]);
    ...
    doc.text(`₹${order.total_amount.toLocaleString()}`, 175, finalY + 8, { align: 'right' });
    
    if (order.discount > 0) {
        doc.setTextColor(239, 68, 68);
        doc.text('Discount:', 130, finalY + 14);
        doc.text(`-₹${order.discount.toLocaleString()}`, 175, finalY + 14, { align: 'right' });
        doc.setTextColor(0, 0, 0);
    }
    ...
    doc.text(`₹${(order.net_amount || order.total_amount).toLocaleString()}`, 175, finalY + 22, { align: 'right' });
